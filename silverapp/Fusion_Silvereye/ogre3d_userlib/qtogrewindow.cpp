...
Modificado por JM
No se le ha cambiado nada de fondo tho.
...
QTOgreWindow::~QTOgreWindow()
{
	...
	qDebug() << "-------------- destructor qtogrewindow--------------------- ";
}
...
void QTOgreWindow::initialize()
{
	...
	*/
	...
	// JM
	Ogre::Log * ogreLog = Ogre::LogManager::getSingleton().getDefaultLog();
	ogreLog->setLogDetail( Ogre::LL_LOW  );
	// end JM

	Ogre::ConfigFile ogreConfig;

	// jm uncommented
	//Commented out for simplicity but should you need to initialize resources you can do so normally.
